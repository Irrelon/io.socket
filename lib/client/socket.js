...
(function (options) {
  ...
  var JSONP = function (target) {
    ...
    this.pollFunc = function () {
      ...
      self.poll.timeOut = window.setTimeout(self.pollFunc, options.pollTimeout || 25000);
  ...
  JSONP.prototype.send = function (data) {
    ...
    // Test at some point.
    el.src = this.url + '/' + Util.generateRandomString() + '?' + window.encodeURIComponent(data);
  ...
  var Socket = function (transport, url) {
    ...
    this.url = normalizeUrl(url) + '/' + transport + '/' + this.id;
