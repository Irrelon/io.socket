...
WebSocket.frame = function (data) {
  var startOffset = 2, firstByte = 0x80, secondByte = data.length;
  if (data.length > 125) {
    startOffset += 2;
    secondByte += 126;
  }
  ...
  var buf = new Buffer(data.length * 2 + startOffset);
  buf[0] = firstByte;
  buf[1] = 0x10 | secondByte;
  buf.write(data, startOffset);
  return buf;
};

WebSocket.prototype.write = function (data) {
  this.socket.write(WebSocket.frame(data), 'binary');
};
